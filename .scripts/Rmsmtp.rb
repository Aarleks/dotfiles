...
# Trims the "!rm" marker string from the beginning or end of the input. Returns
# true if the input was trimmed.
def trim_Rmarkdown_markers!(input)
  trimmed = !!input.sub!(RMARKDOWN_MARKER_AT_START, '')
  !!input.sub!(RMARKDOWN_MARKER_AT_END, '') || trimmed
end

RMARKDOWN_MARKER_AT_START = /\A\s*^[ \t]*!rm[ \t]*$/
RMARKDOWN_MARKER_AT_END = /^[ \t]*!rm[ \t]*$\s*\z/
...
def process_leaf(part)
  if part.content_type =~ %r{\Atext/plain\b} && part.content_disposition == 'inline'
    ...
    else if trim_Rmarkdown_markers!(content)
      part.body = content
      if html = Rmarkdown(content)
        html_part = mail::part.new
        html_part.content_type = 'text/html'
        html_part.body = html
        return html_part
